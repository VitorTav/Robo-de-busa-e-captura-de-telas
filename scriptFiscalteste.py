...
def digitar_caminho_arquivo(caminho):
    # Aguarda 2 segundos para garantir que a janela/campo de entrada esteja em foco
    time.sleep(2)
    pyautogui.write(caminho)  # Digita o caminho do arquivo
    time.sleep(1)  # Aguarda 1 segundo para garantir que o caminho seja digitado corretamente
    pyautogui.press('enter')  # Pressiona a tecla Enter
    print(f"O arquivo está na pasta do share point: {caminho}")
    

def processar_arquivos_no_diretorio(diretorio):
    for arquivo in os.listdir(diretorio):
        caminho_completo = os.path.join(diretorio, arquivo)
        if os.path.isfile(caminho_completo):
            # Certifique-se de que a janela do explorador de arquivos esteja em foco
            pyautogui.click(x=1048, y=48)  # Ajuste as coordenadas conforme necessário
            time.sleep(1)
            digitar_caminho_arquivo(caminho_completo)
            time.sleep(2)  # Aguarda 2 segundos entre cada arquivo para garantir que o upload seja processado


def clicar_na_posicao(x, y):
...
for arquivo in os.listdir(diretorio_cte):
    if os.path.isfile(os.path.join(diretorio_cte, arquivo)):
        caminho_arquivo = os.path.join(diretorio_cte, arquivo)
        digitar_caminho_arquivo(caminho_arquivo)
time.sleep(60)
...
for arquivo in os.listdir(diretorio_nfe):
    if os.path.isfile(os.path.join(diretorio_nfe,arquivo)):
        digitar_caminho_arquivo(diretorio_nfe+arquivo)
time.sleep(60)
